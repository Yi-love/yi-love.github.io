...
首先会通过对传入的参数使用`normalizeArgs`函数进行序列化。在Linux中`backlog`表示已完成(`ESTABLISHED`)且未`accept`的队列大小。
...
### 3.3 listen函数
这个普通函数的工作就是，判断到底是使用哪条进程来创建这个服务器。
...
有客户端请求连接怎么办，这就是这个函数的作用。创建`Socket`对象，连接客户端。
```js
/**
